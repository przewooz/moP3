...
int  CLog::iGetLineNumber(int  iLogOffset)
{
	if (iLogOffset < 0)  return(-1);
	if (iLogOffset >= v_messages.size())  return(0);

	//CString  s_buf;
	//s_buf.Format("offset: %d; lines: %d", iLogOffset, v_messages.at(iLogOffset).size());
	//::MessageBox(NULL, s_buf, s_buf, MB_OK);

	return(v_messages.at(iLogOffset).size());
}//int  CLog::iGetLineNumber(int  iLogOffset)
